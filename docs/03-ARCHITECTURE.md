...
Dokumen ini menjelaskan arsitektur sistem, alur data (_data pipeline_), topologi infrastruktur cloud, dan model keamanan untuk website resmi UKM Coding.
...
- Mengimplementasikan `ClientRouter` untuk navigasi antar-halaman yang mulus (_instant transition_) tanpa memuat ulang aset global.
...
- Menghosting berkas statis di ratusan data center tepi (_edge_) di seluruh dunia.
...
- Repositori utama (_source of truth_) untuk seluruh kode sumber dan riwayat komit.
- Pipeline otomatisasi di `.github/workflows/deploy.yml` yang menjalankan pemeriksaan kualitas kode (_type-check, formatting, linting, testing, Lighthouse CI_) sebelum proses rilis ke Cloudflare Pages.
...
2. **Drafting**: Setiap perubahan otomatis tersimpan sebagai _draft_ aman yang tidak terlihat oleh publik.
3. **Penerbitan**: Saat editor menekan tombol **Publish**, dokumen masuk ke status _published_ di Sanity Content Lake.
...
- **Zero Client Privilege**: Frontend website hanya memiliki akses _read-only_ ke dataset publik Sanity. Token dengan hak akses _write_ tidak pernah disimpan di kode frontend.
