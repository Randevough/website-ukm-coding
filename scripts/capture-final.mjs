...
const OUT_DIR = path.resolve("previews/final");
...
async function run() {
  console.log("Launching browser for final refined visual capture...");
  ...
  for (const route of routes) {
    ...
    for (const vp of viewports) {
      ...
      // Trigger motion reveals so all content is fully rendered
      await page.evaluate(() => {
  ...
  await browser.close();
  console.log("\nCapture complete! All refined screenshots in previews/final/");
}
